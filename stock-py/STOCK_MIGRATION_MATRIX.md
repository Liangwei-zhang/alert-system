...
- Python 重构仓库：`/home/nico/.openclaw/workspace-main/stock-py`
...
| 部署与运维 | QA / cutover 工具、Docker Compose、root Dockerfile、Nginx scaffold 与 compose + VM 单机 baseline 已补强，但 K8s / HA 路线仍未作为默认交付面 |
| React 前端三端 | 不在 Python 迁移范围 |

一句话结论：

- `stock` 的核心后端能力已经基本迁到 `stock-py`。
- `stock` 作为完整产品和完整运维体系，单机 production baseline 已补到 compose + VM 目标形态，当前剩余重点收敛到真实环境 rehearsal 结果沉淀、更多 runtime 指标消费，以及后续 K8s / HA 演进而不是基础 deploy parity。
...
| 原 stock 功能 | stock 原位置 | stock-py 对应 | 状态 | 缺口 |
|---|---|---|---|---|
| internal monitoring stats / metrics / reset | `server/routes/monitoring.ts`，挂载于 `/api/monitoring/*` | 无等价 public route | 未迁移 | Python 只有通用 `/metrics`，没有 `/api/monitoring/stats`、`/metrics`、`/reset` 这一套内部运维端点 |
...
| 真实 staging baseline / rehearsal 记录 | 原项目仍需人工执行 | Python 侧工具已备齐，但记录未完成 | 未完成 |
...
| PM2 ecosystem | `ops/ecosystem.config.js` | 不存在 | 未迁移 |
| PostgreSQL tuning config | `ops/postgresql.conf.tuning` | 不存在 | 未迁移 |
| K8s/HPA/Prometheus/Grafana baseline | 原 `stock` 也未完全体现为 K8s 成品，但部署栈更完整 | `stock-py` 仍在文档 target-state | 未迁移 |

说明：

- `stock-py` 当前 `ops/` 已包含 compose、Nginx、PgBouncer、ClickHouse、MinIO、reports 与 runbooks。
- 当前剩余缺口主要是 K8s / HA 路线与真实环境演练沉淀，而不是 compose + VM 单机 baseline 本身。

## 9. 前端 / 客户端面

| 原 stock 能力 | stock 原位置 | stock-py 状态 | 结论 |
|---|---|---|---|
| `/app` H5 订阅端 | `src/MobileApp.tsx` 及相关页面 | 不在 Python 仓库 | 不在迁移范围 |
| `/platform` 桌面研究台 | `src/App.tsx` | 不在 Python 仓库 | 不在迁移范围 |
| `/admin` 管理前端 | `src/AdminApp.tsx` | 不在 Python 仓库 | 不在迁移范围 |

说明：

- 按 Python 迁移方案，React 前端三端本来就不迁到 Python repo。
- 所以前端不应算作“迁移遗漏”，但也不能说“整个 stock 项目已经完全迁移完”。
...
1. 在真实 staging / canary 环境执行 load baseline、cutover rehearsal，并把 artifact 沉淀到 `ops/reports/*`。
2. 继续补齐更细的运营审计、补偿與容量觀測面，避免 admin parity 停留在 route 對齊。
3. 结合真实运行数据继续微调 broker lag、dispatch failure、archive failure 等告警阈值与消费面板。

### 10.2 中优先级缺口

1. 判断是否还需要恢复更完整的 internal monitoring reset / stats surface，而不只是通用 `/metrics` 与 admin runtime 面。
2. 判断是否还需要保留 PM2 / PostgreSQL tuning config 这类旧式 VM 辅助文件，还是完全以 compose + PgBouncer baseline 为主。
3. 在 HA / 多副本需求真正出现后，再把 K8s manifest / HPA / Prometheus / Grafana 从 roadmap 推进为实际交付物。

### 10.3 生产化缺口

1. 用真 ClickHouse 替换当前 JSONL analytics facade。
2. 用 Kafka / Redpanda 替换当前本地 event-source 假设，补齐 DLQ / retry / replay 策略。
3. 接入外部 S3 / MinIO object storage。
4. 根据真实生产拓扑补齐 K8s / HA deploy template parity。
5. 做真实 staging load baseline、canary / rollback rehearsal、shadow read / dual-write parity 验证。
...
- `stock-py` 已补齐 scheduler 编排、retention 分区维护、runtime 深度指标与 compose + VM 单机部署基线。
- `stock-py` 也尚未完成 production-grade data plane 与切流验证。
