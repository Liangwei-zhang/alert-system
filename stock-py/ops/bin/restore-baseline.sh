...
if [ -z "$BACKUP_DIR" ] || [ ! -f "$BACKUP_DIR/postgres/stock.dump" ]; then
...
attempts=0
until docker compose -f "$COMPOSE_FILE" exec -T postgres pg_isready -U stock -d stock >/dev/null 2>&1; do
  attempts=$((attempts + 1))
...
docker compose -f "$COMPOSE_FILE" exec -T postgres psql -U stock -d stock -c 'DROP SCHEMA IF EXISTS public CASCADE; CREATE SCHEMA public;'
docker compose -f "$COMPOSE_FILE" exec -T postgres /bin/sh -c 'pg_restore -U stock -d stock --clean --if-exists --no-owner --no-privileges' < "$BACKUP_DIR/postgres/stock.dump"
