...
from infra.cache.push_devices_cache import (
    ...
    cache_push_devices,
    get_cached_push_devices,
    get_or_load_push_devices,
...
class PushDevicesCacheTest(unittest.TestCase):
    def test_cache_round_trip_uses_redis_payload(self) -> None:
        client = FakeRedisClient()

        async def run_test() -> None:
            with patch(
                "infra.cache.push_devices_cache.get_redis",
                AsyncMock(return_value=client),
            ):
                await cache_push_devices(
                    42,
                    [{"id": "device-row-1", "device_id": "device-1"}],
                    ttl_seconds=30,
                )
                cached = await get_cached_push_devices(42)
            self.assertEqual(cached, [{"id": "device-row-1", "device_id": "device-1"}])
            self.assertEqual(
                json.loads(client.storage["notifications:push-devices:v1:42"])[0]["device_id"],
                "device-1",
            )
    ...
    def test_get_or_load_coalesces_concurrent_misses(self) -> None:
        ...
        async def run_test() -> None:
            ...
            async def loader() -> list[dict]:
                ...
                return [{"id": "device-row-1", "device_id": "device-1"}]
            ...
            with (
                ...
                first = asyncio.create_task(get_or_load_push_devices(42, loader))
                await loader_started.wait()
                second = asyncio.create_task(get_or_load_push_devices(42, loader))
                await asyncio.sleep(0)
                release_loader.set()
                first_payload, second_payload = await asyncio.gather(first, second)

            self.assertEqual(loader_calls, 1)
            self.assertEqual(first_payload, [{"id": "device-row-1", "device_id": "device-1"}])
    ...
    def test_get_or_load_uses_remote_fill_when_distributed_lock_is_held(self) -> None:
        client = FakeRedisClient()
        lock_key = "notifications:push-devices:v1:42:fill-lock"
        ...
        async def run_test() -> None:
            ...
            async def remote_fill() -> None:
                ...
                client.storage["notifications:push-devices:v1:42"] = json.dumps(
                    [{"id": "device-row-9", "device_id": "device-9"}]
                )
            ...
            with (
                ...
                payload = await get_or_load_push_devices(42, loader)
                await remote_task

            self.assertEqual(payload, [{"id": "device-row-9", "device_id": "device-9"}])
    ...
    def test_pending_invalidation_ops_are_deduplicated_on_apply(self) -> None:
        session = FakeSession()
        schedule_invalidate_push_devices(session, 42)
        schedule_invalidate_push_devices(session, [42, 43])
        operations = pop_pending_push_devices_cache_operations(session)

        async def run_test() -> None:
            with patch(
                "infra.cache.push_devices_cache.invalidate_push_devices",
                AsyncMock(),
            ) as invalidator:
                await apply_push_devices_cache_operations(operations)
